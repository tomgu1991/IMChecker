...
int pkcs8_main(int argc, char **argv)
{
    ...
    OPTION_CHOICE o;
    int nocrypt = 0, ret = 1, iter = 2048;
    int informat = (5 | 0x8000), outformat = (5 | 0x8000), topk8 = 0, pbe_nid = -1;
 ...
 opthelp:
        ...
        case OPT_HELP:
            ...
            goto end;
        case OPT_INFORM:
            if (!opt_format(opt_arg(), (1L << 1), &informat))
                goto opthelp;
            break;
        case OPT_IN:
            infile = opt_arg();
            break;
        case OPT_OUTFORM:
            if (!opt_format(opt_arg(), (1L << 1), &outformat))
                goto opthelp;
            break;
        case OPT_OUT:
            outfile = opt_arg();
        ...
        case OPT_V2PRF:
            pbe_nid = OBJ_txt2nid(opt_arg());
            if (!EVP_PBE_find(0x1, pbe_nid, ((void*)0), ((void*)0), 0)) {
                BIO_printf(bio_err,
    ...
    BIO_free_all(out);
    BIO_free(in);
    CRYPTO_free(passin, "apps/pkcs8.c", 355);
    CRYPTO_free(passout, "apps/pkcs8.c", 356);
