...
# 23 "apps/engine.c" 2
# 1 "include/openssl/store.h" 1
# 16 "include/openssl/store.h"
# 1 "include/openssl/storeerr.h" 1
# 17 "include/openssl/storeerr.h"
int ERR_load_OSSL_STORE_strings(void);
# 17 "include/openssl/store.h" 2
...
static int append_buf(char **buf, int *size, const char *s)
{
    ...
    if (p == ((void*)0)) {
        *size = ((len + expand - 1) / expand) * expand;
        ...
        if (len > *size) {
            *size = ((len + expand - 1) / expand) * expand;
            p = CRYPTO_realloc(p, *size, "apps/engine.c", 67);
            if (p == ((void*)0)) {
                CRYPTO_free(*buf, "apps/engine.c", 69);
                *buf = ((void*)0);
                return 0;
            }
            *buf = p;
...
static int util_flags(BIO *out, unsigned int flags, const char *indent)
{
    ...
    if (flags & (unsigned int)0x0008) {
        BIO_printf(out, "[Internal] ");
    }

    if (flags & (unsigned int)0x0001) {
    ...
    flags = flags & ~(unsigned int)0x0001 &
        ~(unsigned int)0x0002 &
        ~(unsigned int)0x0004 & ~(unsigned int)0x0008;
    if (flags) {
        if (started)
            BIO_printf(out, "|");
...
static int util_verbose(ENGINE *e, int verbose, BIO *out, const char *indent)
{
    ...
    int flags;
    int xpos = 0;
    struct stack_st_OPENSSL_STRING *cmds = ((void*)0);
    if (!ENGINE_ctrl(e, 10, 0, ((void*)0), ((void*)0)) ||
        ((num = ENGINE_ctrl(e, 11,
                            0, ((void*)0), ((void*)0))) <= 0)) {
        return 1;
    }
    ...
    do {
        ...
        if ((flags = ENGINE_ctrl(e, 18, num,
                                 ((void*)0), ((void*)0))) < 0)
            goto err;
        if (!(flags & (unsigned int)0x0008) || verbose >= 4) {

            if ((len = ENGINE_ctrl(e, 14, num,
                                   ((void*)0), ((void*)0))) <= 0)
                goto err;
            name = app_malloc(len + 1, "name buffer");
            if (ENGINE_ctrl(e, 15, num, name,
                            ((void*)0)) <= 0)
                goto err;

            if ((len = ENGINE_ctrl(e, 16, num,
                                   ((void*)0), ((void*)0))) < 0)
                goto err;
            if (len > 0) {
                desc = app_malloc(len + 1, "description buffer");
                if (ENGINE_ctrl(e, 17, num, desc,
                                ((void*)0)) <= 0)
                    goto err;
            }
            ...
            if (verbose == 1) {
                ...
                BIO_printf(out, "%s: %s\n", name,
                           (desc == ((void*)0)) ? "<no description>" : desc);
        ...
        CRYPTO_free(name, "apps/engine.c", 217);
        name = ((void*)0);
        CRYPTO_free(desc, "apps/engine.c", 219);
        desc = ((void*)0);

        num = ENGINE_ctrl(e, 12, num, ((void*)0), ((void*)0));
    } while (num > 0);
    if (xpos > 0)
        BIO_printf(out, "\n");
    ret = 1;
 err:
    ...
    CRYPTO_free(name, "apps/engine.c", 229);
    CRYPTO_free(desc, "apps/engine.c", 230);
    return ret;
}

static void util_do_cmds(ENGINE *e, struct stack_st_OPENSSL_STRING *cmds,
                         BIO *out, const char *indent)
{
    ...
    for (loop = 0; loop < num; loop++) {
        ...
        if ((arg = strstr(cmd, ":")) == ((void*)0)) {
            if (!ENGINE_ctrl_cmd_string(e, cmd, ((void*)0), 0))
                res = 0;
        } else {
            if ((int)(arg - cmd) > 254) {
...
int engine_main(int argc, char **argv)
{
    ...
    ENGINE *e;
    struct stack_st_OPENSSL_CSTRING *engines = sk_OPENSSL_CSTRING_new_null();
    struct stack_st_OPENSSL_STRING *pre_cmds = sk_OPENSSL_STRING_new_null();
    struct stack_st_OPENSSL_STRING *post_cmds = sk_OPENSSL_STRING_new_null();
    BIO *out;
    const char *indent = "     ";
    OPTION_CHOICE o;
    char *prog;
    ...
    out = dup_bio_out((1 | 0x8000));
    if (engines == ((void*)0) || pre_cmds == ((void*)0) || post_cmds == ((void*)0))
        goto end;



    prog = argv[0];
    while ((argv1 = argv[1]) != ((void*)0) && *argv1 != '-') {
    ...
    if (sk_OPENSSL_CSTRING_num(engines) == 0) {
        for (e = ENGINE_get_first(); e != ((void*)0); e = ENGINE_get_next(e)) {
    ...
    for (i = 0; i < sk_OPENSSL_CSTRING_num(engines); i++) {
        ...
        if ((e = ENGINE_by_id(id)) != ((void*)0)) {
            ...
            if (list_cap) {
                ...
                if (ENGINE_get_RSA(e) != ((void*)0)
                    && !append_buf(&cap_buf, &cap_size, "RSA"))
                    goto end;
                if (ENGINE_get_DSA(e) != ((void*)0)
                    && !append_buf(&cap_buf, &cap_size, "DSA"))
                    goto end;
                if (ENGINE_get_DH(e) != ((void*)0)
                    && !append_buf(&cap_buf, &cap_size, "DH"))
                    goto end;
                if (ENGINE_get_RAND(e) != ((void*)0)
 ...
 skip_pmeths:
                ...
                if (cap_buf != ((void*)0) && (*cap_buf != '\0'))
                    BIO_printf(out, " [%s]\n", cap_buf);

                CRYPTO_free(cap_buf, "apps/engine.c", 454);
            }
            if (test_avail) {
