...
int genpkey_main(int argc, char **argv)
{
    ...
    OPTION_CHOICE o;
    int outformat = (5 | 0x8000), text = 0, ret = 1, rv, do_param = 0;
    int private = 0;
 ...
 opthelp:
        ...
        case OPT_CIPHER:
            if (!opt_cipher(opt_unknown(), &cipher)
                || do_param == 1)
                goto opthelp;
            if ((EVP_CIPHER_flags(cipher) & 0xF0007) == 0x6 ||
                (EVP_CIPHER_flags(cipher) & 0xF0007) == 0x7 ||
                (EVP_CIPHER_flags(cipher) & 0xF0007) == 0x10001 ||
                (EVP_CIPHER_flags(cipher) & 0xF0007) == 0x10003) {
    ...
    BIO_free(in);
    release_engine(e);
    CRYPTO_free(pass, "apps/genpkey.c", 205);
    return ret;
}
