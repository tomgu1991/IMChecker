...
# 22 "apps/rehash.c" 2
# 35 "apps/rehash.c"
# 1 "include/internal/o_dir.h" 1
# 46 "include/internal/o_dir.h"
                      ...
                      unsigned short old_id)
{
    ...
    unsigned int ndx = (type + hash) % (sizeof(hash_table)/sizeof((hash_table)[0]));
    ...
    if (ep == ((void*)0)) {
        ...
        *ep = nilhentry;
        ep->old_id = ~0;
        ep->filename = CRYPTO_strdup(filename, "apps/rehash.c", 151);
        if (bp->last_entry)
...
static int handle_symlink(const char *filename, const char *fullpath)
{
    ...
    if (filename[i++] != '.')
        return -1;
    for (type = (sizeof(suffixes)/sizeof((suffixes)[0])) - 1; type > 0; type--) {
...
{
    struct stack_st_X509_INFO *inf = ((void*)0);
    X509_INFO *x;
    ...
    if ((b = BIO_new_file(fullpath, "r")) == ((void*)0)) {
        BIO_printf(bio_err, "%s: error: skipping %s, cannot open file\n",
                   opt_getprog(), filename);
        errs++;
        goto end;
    }
    inf = PEM_X509_INFO_read_bio(b, ((void*)0), ((void*)0), ((void*)0));
    BIO_free(b);
    if (inf == ((void*)0))
        goto end;
    ...
    if (x->x509 != ((void*)0)) {
        ...
        X509_digest(x->x509, evpmd, digest, ((void*)0));
    } else if (x->crl != ((void*)0)) {
        type = TYPE_CRL;
        name = X509_CRL_get_issuer(x->crl);
        X509_CRL_digest(x->crl, evpmd, digest, ((void*)0));
    } else {
        ++errs;
        goto end;
    }
...
{
    CRYPTO_free(s, "apps/rehash.c", 280);
}
...
static int do_dir(const char *dirname, enum Hash h)
{
    BUCKET *bp, *nextbp;
    HENTRY *ep, *nextep;
    OPENSSL_DIR_CTX *d = ((void*)0);
    struct stat st;
    unsigned char idmask[256 / 8];
    int n, numfiles, nextid, buflen, errs = 0;
    size_t i;
    const char *pathsep;
    const char *filename;
    char *buf, *copy = ((void*)0);
    struct stack_st_OPENSSL_STRING *files = ((void*)0);

    if (app_access(dirname, 2) < 0) {
    ...
    if ((files = sk_OPENSSL_STRING_new_null()) == ((void*)0)) {
        BIO_printf(bio_err, "Skipping %s, out of memory\n", dirname);
        errs = 1;
        goto err;
    }
    while ((filename = OPENSSL_DIR_read(&d, dirname)) != ((void*)0)) {
        if ((copy = CRYPTO_strdup(filename, "apps/rehash.c", 332)) == ((void*)0)
                || sk_OPENSSL_STRING_push(files, copy) == 0) {
            CRYPTO_free(copy, "apps/rehash.c", 334);
    ...
    for (n = 0; n < numfiles; ++n) {
        ...
        if (lstat(buf, &st) < 0)
            continue;
        if (((((st.st_mode)) & 0170000) == (0120000)) && handle_symlink(filename, buf) == 0)
            continue;
            ...
            for (ep = bp->first_entry; ep; ep = nextep) {
                ...
                if (ep->old_id < bp->num_needed) {
                    ...
                    if (verbose)
                        BIO_printf(bio_out, "link %s -> %s\n",
                                   ep->filename, &buf[n]);
                    if (unlink(buf) < 0 && (*__errno_location ()) != 2) {
                        BIO_printf(bio_err,
                                   "%s: Can't unlink %s, %s\n",
                                   opt_getprog(), buf, strerror((*__errno_location ())));
                        errs++;
                    }
                ...
                CRYPTO_free(ep->filename, "apps/rehash.c", 414);
                CRYPTO_free(ep, "apps/rehash.c", 415);
            }
            CRYPTO_free(bp, "apps/rehash.c", 417);
        }
        hash_table[i] = ((void*)0);
    }
 ...
 err:
    ...
    CRYPTO_free(buf, "apps/rehash.c", 424);
    return errs;
}
...
int rehash_main(int argc, char **argv)
{
    ...
    if (*argv != ((void*)0)) {
        ...
        char lsc[2] = { ':', '\0' };
        m = CRYPTO_strdup(env, "apps/rehash.c", 489);
        for (e = strtok(m, lsc); e != ((void*)0); e = strtok(((void*)0), lsc))
            errs += do_dir(e, h);
        CRYPTO_free(m, "apps/rehash.c", 492);
    } else {
