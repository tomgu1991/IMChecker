...
# 16 "apps/s_socket.c" 2
# 1 "include/openssl/opensslconf.h" 1
# 17 "apps/s_socket.c" 2
# 31 "apps/s_socket.c"
# 1 "apps/apps.h" 1
# 13 "apps/apps.h"
# 1 "./e_os.h" 1
...
# 34 "apps/s_socket.c" 2





BIO_ADDR *ourpeer = ((void*)0);
# 62 "apps/s_socket.c"
int init_client(int *sock, const char *host, const char *port,
                const char *bindhost, const char *bindport,
                int family, int type, int protocol)
{
    ...
    for (ai = res; ai != ((void*)0); ai = BIO_ADDRINFO_next(ai)) {
        ...
        (void)(((family == 0 || family == BIO_ADDRINFO_family(ai)) && (type == 0 || type == BIO_ADDRINFO_socktype(ai)) && (protocol == 0 || protocol == BIO_ADDRINFO_protocol(ai))) ? 0 : (OPENSSL_die("assertion failed: " "(family == AF_UNSPEC || family == BIO_ADDRINFO_family(ai)) && (type == 0 || type == BIO_ADDRINFO_socktype(ai)) && (protocol == 0 || protocol == BIO_ADDRINFO_protocol(ai))", "apps/s_socket.c", 101), 1));
    ...
    if (*sock == (-1)) {
        if (bindaddr != ((void*)0) && !found) {
            BIO_printf(bio_err, "Can't bind %saddress for %s%s%s\n",
                       BIO_ADDRINFO_family(res) == 10 ? "IPv6 " :
                       BIO_ADDRINFO_family(res) == 2 ? "IPv4 " :
                       BIO_ADDRINFO_family(res) == 1 ? "unix " : "",
              ...
              unsigned char *context, int naccept, BIO *bio_s_out)
{
    ...
    (void)(((family == 0 || family == BIO_ADDRINFO_family(res)) && (type == 0 || type == BIO_ADDRINFO_socktype(res)) && (protocol == 0 || protocol == BIO_ADDRINFO_protocol(res))) ? 0 : (OPENSSL_die("assertion failed: " "(family == AF_UNSPEC || family == BIO_ADDRINFO_family(res)) && (type == 0 || type == BIO_ADDRINFO_socktype(res)) && (protocol == 0 || protocol == BIO_ADDRINFO_protocol(res))", "apps/s_socket.c", 233), 1));
    ...
    if (next != ((void*)0)
        ...
        if (sock_family == 2
                && BIO_ADDRINFO_family(next) == 10) {
            sock_family = 10;
            sock_address = BIO_ADDRINFO_address(next);
        } else if (sock_family == 10
                   && BIO_ADDRINFO_family(next) == 2) {
            sock_options &= ~0x02;
        }
    }
...
# 284 "apps/s_socket.c"
    ...
    if (sock_port == 0) {
        ...
        CRYPTO_free(hostname, "apps/s_socket.c", 308);
        CRYPTO_free(service, "apps/s_socket.c", 309);
...
# 357 "apps/s_socket.c"
            ...
            do {
                ...
                ((void) (((&readfds)->__fds_bits)[((sock) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((sock) % (8 * (int) sizeof (__fd_mask)))))));
            } while (select(sock + 1, &readfds, ((void*)0), ((void*)0), &timeout) > 0
                     && read((sock),(sink),(sizeof(sink))) > 0);
