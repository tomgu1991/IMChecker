...
static int check(X509_STORE *ctx, const char *file,
                 struct stack_st_X509 *uchain, struct stack_st_X509 *tchain,
                 struct stack_st_X509_CRL *crls, int show_chain);
...
typedef enum OPTION_choice {
    ...
    OPT_V__FIRST=2000, OPT_V_POLICY, OPT_V_PURPOSE, OPT_V_VERIFY_NAME, OPT_V_VERIFY_DEPTH, OPT_V_ATTIME, OPT_V_VERIFY_HOSTNAME, OPT_V_VERIFY_EMAIL, OPT_V_VERIFY_IP, OPT_V_IGNORE_CRITICAL, OPT_V_ISSUER_CHECKS, OPT_V_CRL_CHECK, OPT_V_CRL_CHECK_ALL, OPT_V_POLICY_CHECK, OPT_V_EXPLICIT_POLICY, OPT_V_INHIBIT_ANY, OPT_V_INHIBIT_MAP, OPT_V_X509_STRICT, OPT_V_EXTENDED_CRL, OPT_V_USE_DELTAS, OPT_V_POLICY_PRINT, OPT_V_CHECK_SS_SIG, OPT_V_TRUSTED_FIRST, OPT_V_SUITEB_128_ONLY, OPT_V_SUITEB_128, OPT_V_SUITEB_192, OPT_V_PARTIAL_CHAIN, OPT_V_NO_ALT_CHAINS, OPT_V_NO_CHECK_TIME, OPT_V_VERIFY_AUTH_LEVEL, OPT_V_ALLOW_PROXY_CERTS, OPT_V__LAST, OPT_NAMEOPT,
    OPT_VERBOSE
} OPTION_CHOICE;
...
{
    ENGINE *e = ((void*)0);
    struct stack_st_X509 *untrusted = ((void*)0), *trusted = ((void*)0);
    struct stack_st_X509_CRL *crls = ((void*)0);
    X509_STORE *store = ((void*)0);
    X509_VERIFY_PARAM *vpm = ((void*)0);
    const char *prog, *CApath = ((void*)0), *CAfile = ((void*)0);
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_UNTRUSTED:
            ...
            if (!load_certs(opt_arg(), &untrusted, (5 | 0x8000), ((void*)0),
        ...
        case OPT_TRUSTED:
            ...
            noCAfile = 1;
            noCApath = 1;
            if (!load_certs(opt_arg(), &trusted, (5 | 0x8000), ((void*)0),
                            "trusted certificates"))
                goto end;
            break;
        case OPT_CRLFILE:

            if (!load_crls(opt_arg(), &crls, (5 | 0x8000), ((void*)0),
                           "other CRLs"))
                goto end;
            break;
        case OPT_CRL_DOWNLOAD:
            ...
            break;
        case OPT_ENGINE:
            if ((e = setup_engine(opt_arg(), 0)) == ((void*)0)) {
    ...
    argc = opt_num_rest();
    argv = opt_rest();
    if (trusted != ((void*)0) && (CAfile || CApath)) {
        BIO_printf(bio_err,
    ...
    ret = 0;
    if (argc < 1) {
        if (check(store, ((void*)0), untrusted, trusted, crls, show_chain) != 1)
            ret = -1;
    } else {
        for (i = 0; i < argc; i++)
                 ...
                 struct stack_st_X509_CRL *crls, int show_chain)
{
    ...
    x = load_cert(file, (5 | 0x8000), "certificate file");
    if (x == ((void*)0))
        goto end;
    ...
    if (i > 0 && X509_STORE_CTX_get_error(csc) == 0) {
        printf("%s: OK\n", (file == ((void*)0)) ? "stdin" : file);
        ret = 1;
        if (show_chain) {
            int j;
