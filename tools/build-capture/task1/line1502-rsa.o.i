...
int rsa_main(int argc, char **argv)
{
    ...
    int i, private = 0;
    int informat = (5 | 0x8000), outformat = (5 | 0x8000), text = 0, check = 0;
 ...
 opthelp:
    ...
    if (check) {
        ...
        if (r == 1) {
            ...
            while ((err = ERR_peek_error()) != 0 &&
                   (int)(((err) >> 24L) & 0x0FFL) == 4 &&
                   (int)(((err) >> 12L) & 0xFFFL) == 160 &&
                   (int)( (err) & 0xFFFL) != (1|64)) {
    ...
    } else if (outformat == 11 || outformat == 12) {
        EVP_PKEY *pk;
        pk = EVP_PKEY_new();
    ...
    BIO_free_all(out);
    RSA_free(rsa);
    CRYPTO_free(passin, "apps/rsa.c", 309);
    CRYPTO_free(passout, "apps/rsa.c", 310);
    return ret;
}
