...
VERIFY_CB_ARGS verify_args = { 0, 0, 0, 0 };


static unsigned char cookie_secret[16];
static int cookie_initialized = 0;

static BIO *bio_keylog = ((void*)0);
...
int verify_callback(int ok, X509_STORE_CTX *ctx)
{
    ...
    if (err == 0 && ok == 2 && !verify_args.quiet)
        policies_print(ctx);
...
int set_cert_stuff(SSL_CTX *ctx, char *cert_file, char *key_file)
{
    if (cert_file != ((void*)0)) {
        ...
        if (key_file == ((void*)0))
            key_file = cert_file;
        if (SSL_CTX_use_PrivateKey_file(ctx, key_file, 1) <= 0) {
                       ...
                       struct stack_st_X509 *chain, int build_chain)
{
    ...
    if (chain && !SSL_CTX_ctrl(ctx,88,1,(char *)(chain))) {
        BIO_printf(bio_err, "error setting certificate chain\n");
        ERR_print_errors(bio_err);
        return 0;
    }
    if (build_chain && !SSL_CTX_ctrl(ctx,105, chflags, ((void*)0))) {
...
static void ssl_print_client_cert_types(BIO *bio, SSL *s)
{
    ...
    int i;
    int cert_type_num = SSL_ctrl(s, 103, 0, (char *)(&p));
    if (!cert_type_num)
        return;
...
int ssl_print_point_formats(BIO *out, SSL *s)
{
    int i, nformats;
    const char *pformats;
    nformats = SSL_ctrl(s,111,0,&pformats);
    if (nformats <= 0)
        return 1;
...
int ssl_print_groups(BIO *out, SSL *s, int noshared)
{
    ...
    ngroups = SSL_ctrl(s,90,0,(char *)(((void*)0)));
    if (ngroups <= 0)
        return 1;
    groups = app_malloc(ngroups * sizeof(int), "groups to print");
    SSL_ctrl(s,90,0,(char *)(groups));
    ...
    CRYPTO_free(groups, "apps/s_cb.c", 370);
    if (noshared) {
        BIO_puts(out, "\n");
        return 1;
    }
    BIO_puts(out, "\nShared Elliptic groups: ");
    ngroups = SSL_ctrl(s,93,-1,((void*)0));
    for (i = 0; i < ngroups; i++) {
        if (i)
            BIO_puts(out, ":");
        nid = SSL_ctrl(s,93,i,((void*)0));
                             ...
                             unsigned int *cookie_len)
{
    ...
    if (!cookie_initialized) {
        if (RAND_bytes(cookie_secret, 16) <= 0) {
    ...
    if (SSL_is_dtls(ssl)) {
        ...
        (void)(int)BIO_ctrl(SSL_get_rbio(ssl), 46, 0, (char *)(peer));
    } else {
        peer = ourpeer;
    }
    ...
    (void)((length != 0) ? 0 : (OPENSSL_die("assertion failed: " "length != 0", "apps/s_cb.c", 733), 1));
    ...
    HMAC(EVP_sha1(), cookie_secret, 16,
         buffer, length, cookie, cookie_len);

    CRYPTO_free(buffer, "apps/s_cb.c", 745);
    BIO_ADDR_free(lpeer);
...
static STRINT_PAIR chain_flags[] = {
    {"Overall Validity", 0x1},
    {"Sign with EE key", 0x2},
    {"EE signature", 0x10},
    {"CA signature", 0x20},
    {"EE key parameters", 0x40},
    {"CA key parameters", 0x80},
    {"Explicitly sign with EE key", 0x100},
    {"Issuer Name", 0x200},
    {"Certificate Type", 0x400},
    {((void*)0)}
};
...
static void print_chain_flags(SSL *s, int flags)
{
    ...
    if (SSL_ctrl((s),99,(0),((void*)0)) & 0x30000)
        BIO_puts(bio_err, flags & 0x800 ? "OK\n" : "NOT OK\n");
    else
...
# 852 "apps/s_cb.c"
    ...
    while (exc != ((void*)0)) {
        ...
        if (rv & 0x1) {
            ...
            if (exc->build_chain) {
                if (!SSL_ctrl(ssl,105, 0, ((void*)0)))
                    return 0;
            } else if (exc->chain != ((void*)0)) {
                SSL_ctrl(ssl,88,1,(char *)(exc->chain));
            }
        }
        exc = exc->prev;
    }
...
static int ssl_excert_prepend(SSL_EXCERT **pexc)
{
    ...
    *pexc = exc;
    ...
    if (exc->next) {
        ...
        exc->certform = (5 | 0x8000);
        exc->keyform = (5 | 0x8000);
    }
    return 1;
...
void ssl_excert_free(SSL_EXCERT *exc)
{
    ...
    while (exc) {
        ...
        curr = exc;
        exc = exc->next;
        CRYPTO_free(curr, "apps/s_cb.c", 934);
    }
}
...
int load_excert(SSL_EXCERT **pexc)
{
    ...
    for (; exc; exc = exc->next) {
        ...
        if (exc->chainfile != ((void*)0)) {
            if (!load_certs(exc->chainfile, &exc->chain, (5 | 0x8000), ((void*)0),
                            "Server Chain"))
                return 0;
        }
    }
...
enum range { OPT_X__FIRST=1000, OPT_X_KEY, OPT_X_CERT, OPT_X_CHAIN, OPT_X_CHAIN_BUILD, OPT_X_CERTFORM, OPT_X_KEYFORM, OPT_X__LAST };
...
int args_excert(int opt, SSL_EXCERT **pexc)
{
    ...
    case OPT_X_CHAIN_BUILD:
        ...
        break;
    case OPT_X_CERTFORM:
        if (!opt_format(opt_arg(), (1L << 1), &exc->certform))
            return 0;
        break;
    case OPT_X_KEYFORM:
        if (!opt_format(opt_arg(), (1L << 1), &exc->keyform))
            return 0;
        break;
    }
    return 1;
...
static void print_raw_cipherlist(SSL *s)
{
    ...
    if (!SSL_is_server(s))
        return;
    num = SSL_ctrl(s,110,0,((void*)0));
    (void)((num == 2) ? 0 : (OPENSSL_die("assertion failed: " "num == 2", "apps/s_cb.c", 1050), 1));
    rlistlen = SSL_ctrl(s,110,0,&rlist);
...
void print_verify_detail(SSL *s, BIO *bio)
{
    ...
    if ((mdpth = SSL_get0_dane_authority(s, ((void*)0), &mspki)) >= 0) {
        uint8_t usage, selector, mtype;
        const unsigned char *data = ((void*)0);
        size_t dlen = 0;
        char *hexdata;
...
# 1131 "apps/s_cb.c"
        if (dlen > 12)
            hexdata = hexencode(data + dlen - 12, 12);
        else
            hexdata = hexencode(data, dlen);
        BIO_printf(bio, "DANE TLSA %d %d %d %s%s %s at depth %d\n",
                   usage, selector, mtype,
                   (dlen > 12) ? "..." : "", hexdata,
                   (mspki != ((void*)0)) ? "signed the certificate" :
                   mdpth ? "matched TA certificate" : "matched EE certificate",
                   mdpth);
        CRYPTO_free(hexdata, "apps/s_cb.c", 1141);
    }
}
...
int config_ctx(SSL_CONF_CTX *cctx, struct stack_st_OPENSSL_STRING *str,
               SSL_CTX *ctx)
{
    int i;
...
static int add_crls_store(X509_STORE *st, struct stack_st_X509_CRL *crls)
{
    X509_CRL *crl;
    int i;
...
int ssl_ctx_add_crls(SSL_CTX *ctx, struct stack_st_X509_CRL *crls, int crl_download)
{
    X509_STORE *st;
...
int ssl_load_stores(SSL_CTX *ctx,
                    ...
                    struct stack_st_X509_CRL *crls, int crl_download)
{
    X509_STORE *vfy = ((void*)0), *ch = ((void*)0);
    int rv = 0;
    if (vfyCApath != ((void*)0) || vfyCAfile != ((void*)0)) {
        vfy = X509_STORE_new();
        if (vfy == ((void*)0))
            goto err;
        ...
        SSL_CTX_ctrl(ctx,106,1,(char *)(vfy));
        if (crl_download)
            store_setup_crl_download(vfy);
    }
    if (chCApath != ((void*)0) || chCAfile != ((void*)0)) {
        ch = X509_STORE_new();
        if (ch == ((void*)0))
            goto err;
        if (!X509_STORE_load_locations(ch, chCAfile, chCApath))
            goto err;
        SSL_CTX_ctrl(ctx,107,1,(char *)(ch));
    }
    rv = 1;
 err:
    X509_STORE_free(vfy);
...
static STRINT_PAIR callback_types[] = {
    {"Supported Ciphersuite", (1 | (1 << 16))},
    {"Shared Ciphersuite", (2 | (1 << 16))},
    {"Check Ciphersuite", (3 | (1 << 16))},

    {"Temp DH key bits", (7 | (4 << 16))},

    {"Supported Curve", (4 | (2 << 16))},
    {"Shared Curve", (5 | (2 << 16))},
    {"Check Curve", (6 | (2 << 16))},
    {"Supported Signature Algorithm digest", (11 | (5 << 16))},
    {"Shared Signature Algorithm digest", (12 | (5 << 16))},
    {"Check Signature Algorithm digest", (13 | (5 << 16))},
    {"Signature Algorithm mask", (14 | (5 << 16))},
    {"Certificate chain EE key", (16 | (6 << 16))},
    {"Certificate chain CA key", (17 | (6 << 16))},
    {"Peer Chain EE key", ((16 | (6 << 16)) | 0x1000)},
    {"Peer Chain CA key", ((17 | (6 << 16)) | 0x1000)},
    {"Certificate chain CA digest", (18 | (6 << 16))},
    {"Peer chain CA digest", ((18 | (6 << 16)) | 0x1000)},
    {"SSL compression", (15 | 0)},
    {"Session ticket", (10 | 0)},
    {((void*)0)}
};
...
static void keylog_callback(const SSL *ssl, const char *line)
{
    ...
    (void)(int)BIO_ctrl(bio_keylog,11,0,((void*)0));
}
...
int set_keylog_file(SSL_CTX *ctx, const char *keylog_file)
{
    ...
    if ((int)BIO_ctrl(bio_keylog,133,0,((void*)0)) == 0) {
        BIO_puts(bio_keylog,
...
void print_ca_names(BIO *bio, SSL *s)
{
    ...
    const struct stack_st_X509_NAME *sk = SSL_get0_peer_CA_list(s);
    int i;

    if (sk == ((void*)0) || sk_X509_NAME_num(sk) == 0) {
